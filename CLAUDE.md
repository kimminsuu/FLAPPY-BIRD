...
### 모델 선택 가이드 (필수!)
**사용자가 작업을 요청하면, 코드 작성 전에 반드시 적합한 모델을 먼저 추천할 것.**

| 작업 유형 | 추천 모델 | 예시 |
|-----------|----------|------|
| 설계, 플랜, 아키텍처 결정 | **Opus** | 새 기능 설계, 스테이지 기믹 구상, DB 스키마 변경 |
| 복잡한 디버깅, 다중 파일 리팩토링 | **Opus** | 원인 불명 버그 분석, 대규모 구조 변경 |
| 일반 코딩, 버그 수정, UI 수정 | **Sonnet** | 컴포넌트 수정, 스타일 변경, 간단한 기능 추가 |
| 커밋, PR, 문서 작성 | **Sonnet** | README 작성, 커밋 메시지, PR 생성 |

**응답 형식:** "이 작업은 [작업 유형]이므로 **[모델]** 추천합니다. `/model [모델]`로 전환해주세요."
- 이미 적합한 모델이면 전환 없이 바로 진행
- 사용자가 모델 전환 없이 진행하라고 하면 현재 모델로 작업
